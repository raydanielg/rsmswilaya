...
class DodomaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/dodoma.json');
        if (!file_exists($path)) {
            $this->command?->warn("dodoma.json not found at {$path}, skipping DodomaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode dodoma.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('DodomaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
