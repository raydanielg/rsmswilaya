...
class SimiyuLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/simiyu.json');
        if (!file_exists($path)) {
            $this->command?->warn("simiyu.json not found at {$path}, skipping SimiyuLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode simiyu.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('SimiyuLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
