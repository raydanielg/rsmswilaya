...
class MbeyaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/mbeya.json');
        if (!file_exists($path)) {
            $this->command?->warn("mbeya.json not found at {$path}, skipping MbeyaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode mbeya.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('MbeyaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
