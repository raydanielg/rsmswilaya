...
class TaboraLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/tabora.json');
        if (!file_exists($path)) {
            $this->command?->warn("tabora.json not found at {$path}, skipping TaboraLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode tabora.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('TaboraLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
