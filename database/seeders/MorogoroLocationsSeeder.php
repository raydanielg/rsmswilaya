...
class MorogoroLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/morogoro.json');
        if (!file_exists($path)) {
            $this->command?->warn("morogoro.json not found at {$path}, skipping MorogoroLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode morogoro.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('MorogoroLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
