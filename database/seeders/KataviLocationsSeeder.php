...
class KataviLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/katavi.json');
        if (!file_exists($path)) {
            $this->command?->warn("katavi.json not found at {$path}, skipping KataviLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode katavi.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('KataviLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
