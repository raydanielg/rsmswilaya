...
class MtwaraLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/mtwara.json');
        if (!file_exists($path)) {
            $this->command?->warn("mtwara.json not found at {$path}, skipping MtwaraLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode mtwara.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('MtwaraLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
