...
class ArushaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/arusha.json');
        if (!file_exists($path)) {
            $this->command?->warn("arusha.json not found at {$path}, skipping ArushaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode arusha.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('ArushaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
