...
class KigomaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/kigoma.json');
        if (!file_exists($path)) {
            $this->command?->warn("kigoma.json not found at {$path}, skipping KigomaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode kigoma.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('KigomaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
