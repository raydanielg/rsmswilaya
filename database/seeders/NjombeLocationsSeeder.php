...
class NjombeLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/njombe.json');
        if (!file_exists($path)) {
            $this->command?->warn("njombe.json not found at {$path}, skipping NjombeLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode njombe.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('NjombeLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
