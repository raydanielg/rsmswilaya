...
class TangaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/tanga.json');
        if (!file_exists($path)) {
            $this->command?->warn("tanga.json not found at {$path}, skipping TangaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode tanga.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('TangaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
