...
class SingidaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/singida.json');
        if (!file_exists($path)) {
            $this->command?->warn("singida.json not found at {$path}, skipping SingidaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode singida.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('SingidaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
