...
class LindiLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/lindi.json');
        if (!file_exists($path)) {
            $this->command?->warn("lindi.json not found at {$path}, skipping LindiLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode lindi.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('LindiLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
