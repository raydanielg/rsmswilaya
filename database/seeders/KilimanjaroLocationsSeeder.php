...
class KilimanjaroLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/kilimanjaro.json');
        if (!file_exists($path)) {
            $this->command?->warn("kilimanjaro.json not found at {$path}, skipping KilimanjaroLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode kilimanjaro.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('KilimanjaroLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
