...
class IringaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/iringa.json');
        if (!file_exists($path)) {
            $this->command?->warn("iringa.json not found at {$path}, skipping IringaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode iringa.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('IringaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
