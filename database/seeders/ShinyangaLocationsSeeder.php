...
class ShinyangaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/shinyanga.json');
        if (!file_exists($path)) {
            $this->command?->warn("shinyanga.json not found at {$path}, skipping ShinyangaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode shinyanga.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('ShinyangaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
