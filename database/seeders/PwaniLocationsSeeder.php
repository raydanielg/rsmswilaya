...
class PwaniLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/pwani.json');
        if (!file_exists($path)) {
            $this->command?->warn("pwani.json not found at {$path}, skipping PwaniLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode pwani.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('PwaniLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
