...
class SongweLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/songwe.json');
        if (!file_exists($path)) {
            $this->command?->warn("songwe.json not found at {$path}, skipping SongweLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode songwe.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('SongweLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
