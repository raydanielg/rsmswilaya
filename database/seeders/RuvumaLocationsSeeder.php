...
class RuvumaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/ruvuma.json');
        if (!file_exists($path)) {
            $this->command?->warn("ruvuma.json not found at {$path}, skipping RuvumaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode ruvuma.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('RuvumaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
