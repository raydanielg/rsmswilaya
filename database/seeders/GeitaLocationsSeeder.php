...
class GeitaLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/geita.json');
        if (!file_exists($path)) {
            $this->command?->warn("geita.json not found at {$path}, skipping GeitaLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode geita.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('GeitaLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
