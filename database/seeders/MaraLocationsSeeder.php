...
class MaraLocationsSeeder extends Seeder
{
    public function run(): void
    {
        $path = resource_path('views/locations/mara.json');
        if (!file_exists($path)) {
            $this->command?->warn("mara.json not found at {$path}, skipping MaraLocationsSeeder.");
            return;
        }
        ...
        if (!is_array($data)) {
            $this->command?->error('Failed to decode mara.json; expected an array.');
            return;
        }
        ...
        $this->command?->info('MaraLocationsSeeder completed: '.count($regions).' region(s), '.count($districts).' district(s) processed.');
    }
}
