...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
  <head>
    ...
    <title>New Blog Category • RSMS</title>
      ...
      <div class="flex flex-1 min-h-0">
        ...
        <main class="flex-1 min-w-0 p-4 md:p-6">
          <div class="max-w-screen-sm mx-auto bg-white rounded-md border border-[#ecebea] p-6">
            <h1 class="text-2xl font-semibold mb-4">New Blog Category</h1>
            <form method="POST" action="{{ route('admin.blog.categories.store') }}" class="space-y-4">
              @csrf
              <div>
                <label class="block text-sm font-medium mb-1">Name</label>
                <input type="text" name="name" required class="w-full border border-[#e5e4e2] rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0B6B3A]" placeholder="Announcements" />
                @error('name')<div class="text-sm text-red-600 mt-1">{{ $message }}</div>@enderror
              </div>
              <div class="flex gap-3">
                <a href="/blogs" class="px-4 py-2 rounded border">Cancel</a>
